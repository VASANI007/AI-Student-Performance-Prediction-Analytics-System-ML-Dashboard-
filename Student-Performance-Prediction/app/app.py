...
def generate_insights(data, score):
    ...
    if data["study_hours"] < 5:
        insights.append("📚 Study hours are low.")

    if data["attendance"] < 75:
        insights.append("📅 Low attendance affects performance.")

    if data["sleep_hours"] < 6:
        insights.append("😴 Lack of sleep impacts focus.")

    if data["internet_usage"] > 5:
        insights.append("🌐 Too much internet usage.")
...
def generate_ai_report(data, score):
    ...
    report = f"""
The student is predicted to achieve a score of {score}, indicating a {performance} level of performance.
...
Overall, the student {
    "is performing excellently." if score >= 85 else
    "is doing well but can improve." if score >= 70 else
...
with tab1:
    ...
    if st.button("🚀 Predict"):
        ...
        st.markdown(f"""
            ...
            <h2>🎯 Predicted Score: {result['score']}</h2>
            <h3>📊 Grade: {result['grade']}</h3>
            <p style='font-size:18px;'>Student: {name}</p>
            </div>
            """, unsafe_allow_html=True)
        st.subheader("🧠 AI Insights")
        insights = generate_insights(input_data, result["score"])
        for i in insights:
            st.success(i)
        ...
        report_text = generate_ai_report(input_data, result["score"])
